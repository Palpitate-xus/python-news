...
<el-card class="box-card"  v-for="o in newsList" :key="o.id" shadow="hover">
  <div slot="header" class="clearfix">
    ...
    <el-button type="text" style="float: right;padding: 3px 0;">查看词云</el-button>
    <el-popconfirm
...
export default {
  ...
  methods: {
    ...
    async handleFetchNews () {
      let _this = this
      await axios.post('http://localhost:8000/api/get_top_news', {'id': 200}).then((res) => {
        const dd = res.data
    ...
    async handleDelete (id) {
      await this.$axios({
        method: 'post',
        url: 'http://localhost:8000/api/delete_top_news',
        data: {
          id: id
        }
      })
